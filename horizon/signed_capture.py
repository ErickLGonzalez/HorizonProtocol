...
(event_hash, node_id, node_pos_nm, recv_time_ns, tier) with its per-node key
(HMAC-SHA256 stdlib stand-in; Ed25519 is the deployment target). Without the
node key a co-located adversary cannot forge a receipt - this is what makes the
spoof control (H8-C) meaningful. The key is derived deterministically from
`node_id` (DEMO ONLY, exactly `horizon.stations.demo_registry`'s pattern) -
a real deployment issues independent, unpredictable per-node keys.
...
def measure_now(node_id, node_pos_nm, event_hash, tier):
